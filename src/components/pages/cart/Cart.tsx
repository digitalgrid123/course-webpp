"use client";

import React, { useEffect, useState, useCallback, useMemo } from "react";
import { CourseCard } from "@/components/common/CourseCard/CourseCard";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { PaymentModal } from "@/components/common/PaymentModal/PaymentModal";
import Image from "next/image";
import { Formik, Form, Field, FieldProps } from "formik";
import FormInputField from "@/components/common/Form/FormInput";
import { useSelector, useDispatch } from "react-redux";
import {
  updateCartFromStorage,
  purchaseCourses,
  validateCoupon,
  removeCoupon,
  clearCouponError,
} from "@/store/slices/cartSlice";
import { RootState, AppDispatch } from "@/store";
import toast from "react-hot-toast";
import { ShoppingCart, X, CheckCircle } from "lucide-react";

interface CouponFormValues {
  coupon: string;
}

export default function Cart() {
  const dispatch = useDispatch<AppDispatch>();
  const cart = useSelector((state: RootState) => state.cart);
  const [isPaymentModalOpen, setIsPaymentModalOpen] = useState(false);

  // Memoized calculated values
  const displayAmount = useMemo(() => {
    return cart.coupon.validated ? cart.coupon.newTotal : cart.totalAmount;
  }, [cart.coupon.validated, cart.coupon.newTotal, cart.totalAmount]);

  const isZeroAmount = useMemo(() => displayAmount === 0, [displayAmount]);
  const hasItems = useMemo(() => cart.totalItems > 0, [cart.totalItems]);

  useEffect(() => {
    dispatch(updateCartFromStorage());
  }, [dispatch]);

  // Coupon submission handler
  const handleCouponSubmit = useCallback(
    async (values: CouponFormValues) => {
      if (!values.coupon.trim()) {
        toast.error("אנא הזן קוד קופון");
        return;
      }

      try {
        const result = await dispatch(validateCoupon(values.coupon)).unwrap();

        if (result.status) {
          toast.success(result.message || "הקופון הוחל בהצלחה!");
        } else {
          toast.error(result.message || "קוד קופון לא תקין");
        }
      } catch (error) {
        console.error("Error during coupon validation:", error);
        const errorMessage =
          typeof error === "string" ? error : "אירעה שגיאה בזמן בדיקת הקופון";
        toast.error(errorMessage);
      }
    },
    [dispatch]
  );

  // Remove coupon handler
  const handleRemoveCoupon = useCallback(() => {
    dispatch(removeCoupon());
    toast.success("הקופון הוסר");
  }, [dispatch]);

  // Clear coupon error on input change
  const handleCouponChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      if (cart.coupon.error && e.target.value.trim() !== "") {
        dispatch(clearCouponError());
      }
    },
    [cart.coupon.error, dispatch]
  );

  // Purchase handler for both zero and non-zero amounts
  const handlePurchase = useCallback(async () => {
    if (!hasItems) {
      toast.error("אין קורסים בסל");
      return;
    }

    try {
      const result = await dispatch(purchaseCourses()).unwrap();

      if (result.status) {
        toast.success(result.message || "הרכישה בוצעה בהצלחה!");

        if (result.data?.skipped_courses?.length > 0) {
          toast(
            `חלק מהקורסים לא עובדו: ${result.data.skipped_courses.join(", ")}`
          );
        }
      } else {
        toast.error(result.message || "רכישה נכשלה");
        if (result.errors) {
          toast.error(`שגיאות: ${result.errors.join(", ")}`);
        }
      }
    } catch (error) {
      console.error("Error during purchase:", error);
      const errorMessage =
        typeof error === "string"
          ? error
          : error instanceof Error
          ? error.message
          : "אירעה שגיאה בזמן הרכישה";
      toast.error(errorMessage);
    }
  }, [dispatch, hasItems]);

  // Open payment modal or process directly for zero amount
  const handleOpenPaymentModal = useCallback(() => {
    if (!hasItems) {
      toast.error("אין קורסים בסל");
      return;
    }

    // If amount is zero, process directly without payment modal
    if (isZeroAmount) {
      handlePurchase();
      return;
    }

    // For non-zero amount, show payment modal
    setIsPaymentModalOpen(true);
  }, [hasItems, isZeroAmount, handlePurchase]);

  // Payment success handler
  const handlePaymentSuccess = useCallback(async () => {
    await handlePurchase();
    setIsPaymentModalOpen(false);
  }, [handlePurchase]);

  // Get payment button text based on amount and loading state
  const getPaymentButtonText = useCallback(() => {
    if (cart.purchaseLoading) {
      return "מעבד...";
    }

    if (isZeroAmount) {
      return "קבל קורסים בחינם";
    }

    return `סה״כ לתשלום ${displayAmount}₪`;
  }, [cart.purchaseLoading, isZeroAmount, displayAmount]);

  const getPaymentButtonStyle = useCallback(() => {
    const baseStyle =
      "w-full py-3 rounded-md text-sm font-medium transition-colors duration-200 mt-3";

    if (isZeroAmount) {
      return `${baseStyle} bg-green-600 hover:bg-green-700 text-white`;
    }

    return `${baseStyle} bg-black hover:bg-black/90 text-white`;
  }, [isZeroAmount]);

  const renderEmptyCart = () => (
    <div className="flex flex-col items-center justify-center gap-4">
      <ShoppingCart className="w-20 h-20 text-gray-400" />
      <p className="text-center text-gray-500 text-lg">אין קורסים בסל</p>
    </div>
  );

  const renderCouponSection = () => (
    <div className="mb-3 p-3 bg-white/20 rounded-lg">
      <div className="flex items-center justify-center gap-2 mb-1">
        <CheckCircle className="w-4 h-4" />
        <span className="text-sm font-medium text-charcoal-blue">
          קופון הוחל!
        </span>
        <button
          onClick={handleRemoveCoupon}
          className="text-charcoal-blue hover:text-gray-200 transition-colors"
          aria-label="Remove coupon"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      <div className="text-xs space-y-1">
        <div className="flex justify-between text-charcoal-blue">
          <span>הנחה:</span>
          <span>{cart.coupon.discount}%</span>
        </div>
        <div className="flex justify-between line-through text-charcoal-blue">
          <span>מחיר מקורי:</span>
          <span>{cart.totalAmount}₪</span>
        </div>
      </div>
    </div>
  );

  // Render coupon form
  const renderCouponForm = () => (
    <Formik
      initialValues={{ coupon: "" }}
      onSubmit={handleCouponSubmit}
      enableReinitialize
    >
      {({ submitForm, values }) => (
        <Form className="flex flex-col gap-4">
          <div className="flex gap-2 items-center">
            <div className="flex-1">
              <Field name="coupon">
                {({ field }: FieldProps) => (
                  <FormInputField
                    id="coupon"
                    type="text"
                    label=""
                    placeholder="הזן אותו כאן"
                    {...field}
                    onChange={(e) => {
                      field.onChange(e);
                      handleCouponChange(e);
                    }}
                    isRTL
                    className="bg-gray-100 rounded-lg py-5"
                    disabled={cart.coupon.loading || cart.coupon.validated}
                  />
                )}
              </Field>
            </div>
            {!cart.coupon.validated ? (
              <Button
                type="button"
                onClick={submitForm}
                disabled={cart.coupon.loading || !values.coupon.trim()}
                className="bg-black hover:bg-black/90 text-white px-4 py-3 rounded-md text-sm font-medium whitespace-nowrap h-full transition-colors duration-200"
              >
                {cart.coupon.loading ? "..." : "החל"}
              </Button>
            ) : (
              <Button
                type="button"
                onClick={handleRemoveCoupon}
                className="bg-white hover:bg-gray-100 text-charcoal-blue px-4 py-3 rounded-md text-sm font-medium whitespace-nowrap h-full transition-colors duration-200"
              >
                הסר
              </Button>
            )}
          </div>

          {cart.coupon.error && (
            <div className="text-red-200 text-sm bg-red-600/50 p-2 rounded">
              {cart.coupon.error}
            </div>
          )}
        </Form>
      )}
    </Formik>
  );

  const renderCourseGrid = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {cart.items.map((course) => (
        <CourseCard
          key={course.id}
          id={course.id}
          title={course.name}
          instructor={course.teacher.name}
          image={course.image}
          price={course.price?.toString()}
          textAlign="center"
        />
      ))}
    </div>
  );

  // Render order summary
  const renderOrderSummary = () => (
    <div className="lg:col-span-3">
      <div className="sticky top-6">
        <Card className="bg-amber-gold text-white rounded-2xl p-0 border-0 w-full">
          <CardContent className="p-6 text-center">
            <h2 className="text-lg font-bold mb-2">סיכום הזמנה</h2>
            <div className="flex items-center gap-2 mb-4 justify-center">
              <Image
                src={"/assets/images/icons/module-white.svg"}
                width={16}
                height={16}
                alt="module"
              />
              <p className="text-xs text-white">{cart.totalItems} מוצרים</p>
            </div>

            {cart.coupon.validated && renderCouponSection()}

            <div className="text-right mb-2 text-sm text-white" dir="ltr">
              ?יש לך קופון
            </div>

            {renderCouponForm()}

            <Button
              type="button"
              onClick={handleOpenPaymentModal}
              className={getPaymentButtonStyle()}
              disabled={cart.purchaseLoading}
            >
              {getPaymentButtonText()}
            </Button>
          </CardContent>
        </Card>
      </div>
    </div>
  );

  return (
    <div className="p-6 flex flex-col min-h-screen" dir="rtl">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-semibold text-charcoal-blue">הסל שלי</h1>
      </div>

      <div
        className={`grid grid-cols-1 lg:grid-cols-8 gap-8 ${
          hasItems ? "" : "flex-1"
        }`}
      >
        <div
          className={
            hasItems
              ? "lg:col-span-5"
              : "col-span-full flex items-center justify-center"
          }
        >
          {hasItems ? renderCourseGrid() : renderEmptyCart()}
        </div>

        {hasItems && renderOrderSummary()}
      </div>

      {!isZeroAmount && (
        <PaymentModal
          isOpen={isPaymentModalOpen}
          onClose={() => setIsPaymentModalOpen(false)}
          onPaymentSuccess={handlePaymentSuccess}
          isLoading={cart.purchaseLoading}
        />
      )}
    </div>
  );
}
